...
def fibration_linear(weights, in_clusters, threshold, first_layer = False, bias=None):
    ...
    if bias is not None:
        ...
        collapse_weights = torch.cat((collapse_weights, bias.to(dev_).unsqueeze(1)), dim=1)
...
def opfibration_lstm(weight_ih,weight_hh, out_clusters, threshold, T=5):
    ...
    for t in range(5):
        ...
        gates_fib_cluster_matrix = gates_fib_clusters.reshape(4,-1)
	...
	for pt_file in pt_files:
		...
		for idx, layer in enumerate(layers):
			if isinstance(layer, nn.Conv2d):
				...
				clusters = make_labels_sequential(clusters)
				fibers_vs_time[(idx, str(layer))].append(clusters)
				prev_layer = layer
				
			elif isinstance(layer, nn.Linear):
				if isinstance(prev_layer, nn.Conv2d):
					clusters = np.array([x for x in clusters for _ in range(layer.in_features // prev_layer.out_channels)])
...
def get_opfibers_vs_time(pt_files, threshold, has_epoch_zero=False):
    ...
    if hasattr(model.policy, 'policy'):
        layers = [model.policy.policy.actor] + [model.policy.recurrent]
        # + [layer for layer in model.policy.policy.network.children() if isinstance(layer, (nn.Conv2d, nn.Linear))][::-1]
    else:
        layers = [model.policy.actor] + [
            layer for layer in model.policy.network.children()
            if isinstance(layer, (nn.Conv2d, nn.Linear))
        ][::-1]

    # 1 extra list for the epoch
    opfibers_vs_time = {(len(layers) + i + 1, str(layer)): [] for i, layer in enumerate(layers)}
    ...
    for pt_file in pt_files:
        ...
        for idx, layer in enumerate(layers):
            if isinstance(layer, nn.Conv2d):
                clusters = []
                # Add logic here for conv2d if needed
                opfibers_vs_time[(len(layers) - idx - 1, str(layer))].append(clusters)
                prev_layer = layer

            elif isinstance(layer, nn.Linear):
                weight_data = torch.concat([
                    model.policy.policy.actor.weight.data,
                    model.policy.policy.value_fn.weight.data
                ])
                clusters = opfibration_linear(
                    weights=weight_data,
                ...
                clusters = opfibration_lstm(
                    layer.weight_ih_l0.detach(),
                    layer.weight_hh_l0.detach(),
                    clusters,
                    threshold
                )
                opfibers_vs_time[(len(layers) + idx + 1, str(layer))].append(clusters)
...
def relabel_by_shared_clusters(a: np.ndarray, b: np.ndarray) -> np.ndarray:
	a = np.asarray(a)
	b = np.asarray(b)
	...
	# Group indices by (a[i], b[i]) pairs
	pair_to_indices = defaultdict(list)
	for i in range(n):
		pair_to_indices[(a[i], b[i])].append(i)
...
def get_covers_vs_time(fibers_vs_time, opfibers_vs_time):
	covers_vs_time = {k: [] for k in fibers_vs_time.keys()}
 
	for idx, epoch in enumerate(fibers_vs_time['epoch']):
		for key in fibers_vs_time.keys():
			if key != 'epoch':
				covers_vs_time[key].append(relabel_by_shared_clusters(fibers_vs_time[key][idx], opfibers_vs_time[key][idx]))
		covers_vs_time['epoch'].append(epoch)

	return covers_vs_time

## TODO: rewrite this function to generalize
def get_covers_vs_time_new(opfibers_vs_time, fibers):
    coverings = {}

    # Process LSTM layer
    lstm_key = list(opfibers_vs_time.keys())[0]  # Assuming consistent ordering
    op_lstm = opfibers_vs_time[lstm_key][0]
    fibers_lstm = fibers[list(fibers.keys())[4]][0]  # Adjust index if needed

    lstm_matrix = np.hstack([
        np.array(op_lstm).reshape(-1, 1),
        np.array(fibers_lstm).reshape(-1, 1)
    ])

    lstm_tensor = torch.from_numpy(lstm_matrix).to(dtype=torch.int32)
    coverings['LSTM'] = torch.unique(lstm_tensor, dim=0, return_inverse=True)[1]

    # Process Linear layer
    linear_key = list(opfibers_vs_time.keys())[1]
    op_linear = opfibers_vs_time[linear_key][0]
    fibers_linear = fibers[list(fibers.keys())[3]][0]

    linear_matrix = np.hstack([
        np.array(op_linear).reshape(-1, 1),
        np.array(fibers_linear).reshape(-1, 1)
    ])

    linear_tensor = torch.from_numpy(linear_matrix).to(dtype=torch.int32)
    coverings['Linear'] = torch.unique(linear_tensor, dim=0, return_inverse=True)[1]

    return coverings

def make_labels_sequential(labels):
	unique_labels = np.unique(labels)
	label_map = {old.item(): new for new, old in enumerate(unique_labels)}
	sequential_labels = np.array([label_map[label.item()] for label in labels])
	return sequential_labels




# TODO-->eplain properly

def get_fibers(model, threshold):
...
def get_opfibers(model, threshold):
...
def get_covers(config,epoch,model, threshold_f, threshold_o):
    import os 
    full_path = os.path.join(config.data_dir, config.exp_id)
    
    fiber_dir = os.path.join(full_path,'fibers')
    opfiber_dir = os.path.join(full_path,'opfibers')
    
    os.makedirs(fiber_dir, exist_ok=True)
    os.makedirs(opfiber_dir, exist_ok=True)
    
    fibers = get_fibers(model, threshold_f)
    opfibers = get_opfibers(model, threshold_o)[::-1]

    np.save(f"{fiber_dir}/fibers_{epoch:06d}.npy", fibers)
    np.save(f"{opfiber_dir}/opfibers_{epoch:06d}.npz", opfibers)

    coverings = []

    linear_matrix = np.hstack([
        np.array(opfibers[0]).reshape(-1, 1),
        np.array(fibers[0]).reshape(-1, 1)
    ])

    linear_tensor = torch.from_numpy(linear_matrix).to(dtype=torch.int32)
    coverings.append(torch.unique(linear_tensor, dim=0, return_inverse=True)[1])
    
    # Process Linear layer
    lstm_matrix = np.hstack([
        np.array(opfibers[1]).reshape(-1, 1),
        np.array(fibers[1]).reshape(-1, 1)
    ])

    lstm_tensor = torch.from_numpy(lstm_matrix).to(dtype=torch.int32)
    coverings.append(torch.unique(lstm_tensor, dim=0, return_inverse=True)[1])

    # breaking by fibers lstm
    return coverings