...
from coloring import get_covers as get_covers_
from breaking_symmetries import update_model_breaking_symmetry
    ...
    while not experience.full:
        ...
        with profile.eval_forward, torch.no_grad():
            ...
            if lstm_h is not None:
                h = lstm_h[:, env_id]
                c = lstm_c[:, env_id]
                actions, logprob, _, value, (h, c) = policy.get_action_and_value(o_device, (h, c))
                # actions, logprob, _, value, (h, c) = policy(o_device, (h, c))
                lstm_h[:, env_id] = h
                lstm_c[:, env_id] = c
            else:
                actions, logprob, _, value = policy.get_action_and_value(o_device)
...
@pufferlib.utils.profile
def train(data,accumulated_num_features_to_replace,ages,num_new_features_to_replace,collapse_f,collapse_c):
    ### MATTEO:  Setup & GAE Computation
    config, profile, experience = data.config, data.profile, data.experience
    ...
    for epoch in range(config.update_epochs):
        ...
        for mb in range(experience.num_minibatches):
            ...
            ### MATTEO: Forward Pass through the Policy
            with profile.train_forward:
                if experience.lstm_h is not None:
                    _, newlogprob, entropy, newvalue, lstm_state = data.policy.get_action_and_value(
                        obs, state=lstm_state, action=atn)
                    lstm_state = (lstm_state[0].detach(), lstm_state[1].detach())
                else:
                    _, newlogprob, entropy, newvalue = data.policy.get_action_and_value(
            ...
            with profile.train_misc:
                ...
                # Set entropy loss =0 fior both BR and brekout. Run with and without until 3 thousands. 
                # Original with and without the netropy.
                # Breaking with and without Entropy.
                loss = pg_loss - config.ent_coef * entropy_loss + v_loss * config.vf_coef


            ### MATTEO: backpropagation 
            with profile.learn:
        ...
        ### MATTEO early stopping condition
    ...
    with profile.train_misc:
        ...
        if data.epoch % config.checkpoint_interval == 0 or done_training:
            ...
            #TODO Save this every episode (both covers and fiber, opfibers)

            
            colors = get_covers_(data.config,data.epoch,data.policy,threshold_f =collapse_f, threshold_o = collapse_c)
            
            ### ==================== OSVALDO & MATTEO:breaking symmetries ==================== ###
            layers_to_break = [data.policy.policy.policy.network[7], data.policy.policy.recurrent]
            dev_ = layers_to_break[0].weight.device
            bounds = [np.sqrt(2) * np.sqrt(3 /layers_to_break[0].in_features), np.sqrt(3 /layers_to_break[1].input_size)] # check
            
   
            if 'beam_rider' in os.path.join(data.config.data_dir, data.config.exp_id):
                maturity_threshold = int(200/config.checkpoint_interval) #(a)10 (for per_cover=0.1) - (b,c)200 (for per_cover=0.5)
            elif 'breakout' in os.path.join(data.config.data_dir, data.config.exp_id):
                 maturity_threshold = int(300/config.checkpoint_interval) #10 (for per_cover=0.1) - 300 (for per_cover=0.5)
                
            #print(num_new_features_to_replace)
            #print(accumulated_num_features_to_replace)
            #print(data.policy.policy.policy.actor)

            if data.epoch > 500:# (for case maturity 200 - per_cover=0.5) - until episode = 3000:
                update_model_breaking_symmetry(layers_to_break,data.policy.policy.policy.actor,data.policy.policy.policy.value_fn, maturity_threshold, colors, dev_, bounds,ages,num_new_features_to_replace,accumulated_num_features_to_replace)

            #print('Espisode-->',data.epoch)
            #print('\n')
            #exit(0)
            ### =====================================================================================###
