...
def train(args, make_env, policy_cls, rnn_cls, wandb,
    ...
    accumulated_num_features_to_replace = [0 for i in range(num_layers_to_break)]
    
    for idx_l in range(num_layers_to_break):
        layer = layers_to_break[idx_l]
        dim = layer.out_features if isinstance(layer, torch.nn.Linear) else layer.hidden_size
        ages.append(torch.zeros(dim, device=dev_))
        num_new_features_to_replace.append(None)
    ##===================================================== ##
    
    while data.global_step < train_config.total_timesteps:
        clean_pufferl.evaluate(data)
        clean_pufferl.train(data,accumulated_num_features_to_replace,ages,num_new_features_to_replace,args['collapse_f'],args['collapse_c'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--collapse_f', type=float, default=0.0,
        help='Collapse level (float between 0 and 2, 0 = disabled)')


    # NEW ARGUMENT: FIBER THRESHOLD. Matteo Serafino
    parser.add_argument('--collapse_c', type=float, default=0.0,
        help='Collapse level (float between 0 and 2, 0 = disabled)')
